...
<div class="modal-body">
    <form class="sliderForm" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <x-input-field label="{{ __('Title') }}" name="title" value="{{ old('title' ) }}" required="true"
                maxLength="250" />
              ...
              <div class="col-md-6">
                <div class="form-group">
                    <label
                        for="username">{{ __('Image') }}</label>
                    <input type='file'
                        onchange="loadFile_slider_image(slider_image)"
                        name="slider_image" id="slider_image"
                        class="@error('slider_image') is-invalid @enderror"
                        style="display:none;" />
                    <button id="output_slider_image" type="button"
                        onclick="document.getElementById('slider_image').click();"
                        value="emad"
                        style="
            width: 190px;
            height: 100px;
